...
<p align="center">
  覆盖采购入库、库存领用、盘点核算、报表分析与多电脑协作的本地化管理系统
</p>
...
Aster 是面向酒店内部物资流转和库存成本管理的 Windows/macOS 桌面系统。系统采用本地数据库存储业务数据，支持单机运行和局域网多电脑协作，帮助酒店统一管理物品档案、供应商、采购批次、出入库单据、库存余额、盘点差异、部门预算、客用品销售、报表导出和备份恢复，适合没有云端数据库、需要本地可控和低维护成本的使用场景。

完整产品范围见 [Aster 执行文档](docs/ASTER_EXECUTION_PLAN.md)。

<p align="center">
  <img src="docs/images/aster-desktop-screenshot.png" alt="Aster 桌面系统截图">
</p>
...
| 同步方式 | 局域网 TLS API + OPAQUE 一次性配对 + 设备/用户双 token |
