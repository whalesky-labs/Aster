...
| Target platforms | Windows x64, macOS Intel |
...
- `.github/workflows/build-desktop.yml` follows the Liberty desktop build flow: it prepares the version, builds macOS Intel and Windows x64 installers through a matrix, and can optionally publish a GitHub Release.
- The macOS job runs on `macos-15-intel` with the `x86_64-apple-darwin` target and generates an Intel/x64 DMG.
