...
const requirements = [
  ...
  },
  {
    ...
    checks: [
      ...
      ["旧酒店表预览有测试覆盖", "importService", "preview_legacy_workbook_extracts_month_items_and_department_issues"],
      ["物品名称为空校验有测试覆盖", "importService", "preview_legacy_workbook_reports_blank_item_name"],
      ["单位单价数量金额和重复行校验有测试覆盖", "importService", "preview_template_workbook_reports_validation_messages"],
      ["公式错误校验有测试覆盖", "importService", "detect_formula_errors_reports_cell_errors"],
      ["部门列无法识别校验有测试覆盖", "importService", "preview_legacy_workbook_warns_unmapped_department_columns"],
      ["通用模板导入有测试覆盖", "importService", "import_template_workbook_creates_missing_department_and_movements"],
  ...
  },
  {
    ...
    checks: [
      ...
      ["前端包含主机服务、发现、配对", "app", "onDiscoverHosts"],
      ...
      ["前端启停基础资料传递 expectedUpdatedAt", "app", "set_item_enabled\", { id, enabled, expectedUpdatedAt }"],
  ...
  },
  {
    ...
    checks: [
      ...
      ["GitHub Release 只收集安装包 artifacts", "githubDesktopBuild", "installer_dirs"],
      ["GitHub Release 校验 macOS DMG asset", "githubDesktopBuild", "macOS DMG release asset missing"],
      ...
      ["README 说明 Windows 由 GitHub Actions 构建", "readme", "Windows release 包由 GitHub Actions"],
