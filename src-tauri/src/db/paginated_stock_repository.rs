...
WHERE (?1 IS NULL OR d.document_type = ?1) AND (?2 IS NULL OR d.outbound_kind = ?2) AND (?3 IS NULL OR strftime('%Y-%m', d.business_date) = ?3) AND (?4 IS NULL OR d.department_id = ?4) AND (?5 IS NULL OR d.supplier_id = ?5)
