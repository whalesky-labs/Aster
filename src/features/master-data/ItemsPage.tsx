...
import { PaginatedTable, Status, TableSearchToolbar } from "../../shared/ui/DataTable";

function optionName(options: OptionRecord[], id?: string | null) {
  return options.find((item) => item.id === id)?.name ?? "-";
}

export function ItemsPage({ canImportItems, canWrite, categories, formatMoney, itemSearch, items, onCreate, onEdit, onExportItems, onImportItems, onSearch, onToggle, suppliers, units }: {
  canImportItems: boolean; canWrite: boolean; categories: OptionRecord[]; itemSearch: string; items: Item[];
  formatMoney: (value: number) => string;
  onCreate: () => void; onEdit: (id: string) => void; onExportItems: () => Promise<void>; onImportItems: () => Promise<void>;
  onSearch: (search: string) => Promise<void>; onToggle: (id: string, enabled: boolean, expectedUpdatedAt: string) => Promise<void>;
  suppliers: OptionRecord[]; units: OptionRecord[];
}) {
  const [search, setSearch] = useState(itemSearch);
  return <section className="table-panel">
    <TableSearchToolbar action={<button className="primary-button" disabled={!canWrite} onClick={onCreate} type="button">新增物品</button>} onSearchChange={setSearch} onSubmit={() => onSearch(search)} placeholder="搜索编码、名称、规格" search={search} searchLabel="关键字" submitLabel="搜索" />
    <div className="table-utility-toolbar"><div className="table-utility-info"><span>物品档案</span><em>{items.length} 条记录</em></div><div className="table-utility-actions"><button className="ghost-button" disabled={!canImportItems} onClick={onImportItems} type="button">导入</button><button className="ghost-button" onClick={onExportItems} type="button">导出</button></div></div>
