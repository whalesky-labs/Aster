...
export function SuppliersPage({ activeSupplier, canWrite, formatDateTime, formatMoney, onCreate, onEdit, onSelect, onToggle, purchaseRecords, suppliers }: {
  ...
  return <section className="table-panel">
    <TableSearchToolbar action={<button className="primary-button" disabled={!canWrite} onClick={onCreate} type="button">新增供应商</button>} extra={tabs} onSearchChange={setSearch} placeholder={activeTab === "suppliers" ? "搜索名称、联系人、电话、地址" : "搜索单号、物品、规格、备注"} search={search} />
